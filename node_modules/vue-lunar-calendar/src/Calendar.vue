  ...
  methods: {
    ...
    initDays() {
      ...
      for (let i = diff - 1; i >= 0; i--) {
        ...
        this.days.push({ dayMoment, isPassive: true , isNow: this.checkCurrentDateNow(dayMoment)});
      }
      for (let i = 1; i <= dayCount; i++) {
        const dayMoment = this.dayOfMonth.clone().date(i);
        let isPassive = this.isPassive(dayMoment);
        this.days.push({ dayMoment, isPassive , isNow: this.checkCurrentDateNow(dayMoment)});
      }
...
.date-year {
  ...
  flex: 1;
  box-shadow: 0px 0px 7px 1px #999999;
}
.boder-date-now{
  border: thin solid;
}
